...
export type FacebookEventCallback<TEvent extends FacebookEventType> = TEvent extends 'xfbl.render'
  ? () => void
  : (response: StatusResponse) => void;

export interface FacebookStaticEvent {
...
export interface ShareOpenGraphDialogParams extends DialogParams {
...
export interface CreateOfferDialogParams extends DialogParams {
  account_id: string;
  display: 'popup';
  method: 'create_offer';
  objective:
...
export interface LeadgenDialogParams extends DialogParams {
  account_id: string;
  display: 'popup';
  method: 'lead_gen';
  page_id: string;
...
export interface InstantExperiencesAdsDialogParams extends DialogParams {
  display: 'popup';
...
export interface InstantExperiencesPreviewDialogParams extends DialogParams {
  display: 'popup';
  method: 'canvas_preview';
  canvas_id: string;
}

export interface CollectionAdsDialogParams
...
export interface ShareDialogResponse extends DialogResponse {
  post_id: string;
}

export interface ShareOpenGraphDialogResponse extends DialogResponse {
  post_id: string;
}

export interface GameRequestDialogResponse extends DialogResponse {
  request: string;
  to: string[];
}

export interface PayDialogResponse extends DialogResponse {
  payment_id: string;
  amount: string;
  currency: string;
  quantity: string;
...
export interface PaymentsLiteDialogResponse extends DialogResponse {
  app_id: number;
  developer_payload?: string;
  payment_id: number;
  product_id: string;
...
export interface LiveDialogResponse extends DialogResponse {
  id: string;
  stream_url: string;
...
export interface CreateOfferDialogResponse extends DialogResponse {
  id: string;
  success: boolean;
}

export interface LeadgenDialogResponse extends DialogResponse {
  formID: string;
  success: boolean;
}

export interface InstantExperiencesAdsDialogResponse extends DialogResponse {
  id: string;
  success: boolean;
}

export interface CollectionAdsDialogResponse
