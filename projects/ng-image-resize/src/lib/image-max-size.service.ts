...
@Injectable()
export class ImageMaxSizeService {

  public timeAtStart?: number;
  public initialFile!: File;

  public constructor(private imageExifService: ImageExifService) {
  }

  public compressImage(file: File,
                       maxSizeInMB: number,
                       ignoreAlpha: boolean = false,
                       logExecutionTime: boolean = false): Observable<any> {
    ...
    let result: Promise<File> = new Promise((resolve, reject) => {
      cvs.toBlob((blob) => {
        if (blob) {
          ...
          reject(new Error('Couldn\'t convert file to Blob.'));
        }
  ...
  private isImgUsingAlpha(imageData: ImageData): boolean {
